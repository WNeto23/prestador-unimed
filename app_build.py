...
try:
    from api_client import (
        listar_prestadores, criar_prestador, atualizar_prestador,
        excluir_prestador, buscar_prestador,
        listar_datas_envio, criar_data, atualizar_data,
        excluir_data, buscar_data,
        listar_log_envios, registrar_log,
        contar_prestadores, contar_datas_ativas, contar_falhas_7dias
    )
    print("✅ Cliente API carregado!")
except ImportError as e:
    print(f"❌ ERRO CRÍTICO: api_client não disponível - {e}")
    sys.exit(1)
...
def formatar_data_entrada(texto: str) -> str | None:
    ...
    for padrao in padroes:
        ...
        if match:
            try:
                g = match.groups()
                dia = g[0].zfill(2); mes = g[1].zfill(2); ano = g[2]
...
def criar_card_metrica(titulo, valor, cor_borda, icone):
    return ft.Container(
        ...
        padding=20, border_radius=12, expand=True)

def criar_quadro_tabela(tabela):
    return ft.Container(
        content=ft.Column([ft.ListView(controls=[tabela], expand=True)], expand=True),
        expand=True,
...
def main(page: ft.Page):
    ...
    # ====================== CAMPOS PRESTADORES ======================
    txt_codigo = ft.TextField(label="Código", border_color=UNIMED_GREEN, dense=True)
    txt_nome   = ft.TextField(label="Nome do Prestador", border_color=UNIMED_GREEN, dense=True)
    txt_email  = ft.TextField(label="E-mail", border_color=UNIMED_GREEN, dense=True)
    dd_tipo_prestador = ft.Dropdown(
        label="Tipo de Prestador", border_color=UNIMED_GREEN, dense=True,
        options=[
    ...
    txt_referencia = ft.TextField(label="Referência (ex: Fevereiro/2026)", border_color=UNIMED_GREEN, dense=True)
    ...
    txt_filtro_datas = ft.TextField(
        label="Filtrar períodos...", hint_text="Referência ou tipo",
        prefix_icon=ft.Icons.SEARCH, border_color=UNIMED_GREEN, dense=True, expand=True,
    ...
    def on_blur_data(e):
        ...
        if valor := formatar_data_entrada(campo.value):
            campo.value = valor; campo.error_text = None
    ...
    async def salvar_data_com_feedback(e):
        ...
        if not all([dd_tipo_envio.value, txt_referencia.value]):
            ...
            page.overlay.append(snack); snack.open = True; page.update(); return
        try:
            fat_ini = parse_data(txt_fat_ini); fat_fim = parse_data(txt_fat_fim)
    ...
    def excluir_prestador_dlg(id: int):
        def confirmar(e):
            try:
                excluir_prestador(id)
                page.dialog.open = False
                atualizar_tabela_prestadores(); atualizar_metricas()
    ...
    def editar_data(id: int):
        ...
        if row:
            ...
            txt_fat_ini.value = fmt_data(row.get("faturamento_inicio"))
            txt_fat_fim.value = fmt_data(row.get("faturamento_fim"))
            txt_rec_ini.value = fmt_data(row.get("recurso_inicio"))
            txt_rec_fim.value = fmt_data(row.get("recurso_fim"))
            txt_guia1.value   = fmt_data(row.get("guia_fisica_1"))
            txt_guia2.value   = fmt_data(row.get("guia_fisica_2"))
            ...
            for c in [txt_fat_ini,txt_fat_fim,txt_rec_ini,txt_rec_fim,
                      txt_guia1,txt_guia2,txt_guia3,txt_guia4,txt_guia5]:
                if c.value == "-": c.value = ""
            dd_status.value = row.get("status","Ativo")
            btn_salvar_data.text = "Atualizar Datas"
            page.update()

    def excluir_data_dlg(id: int):
        def confirmar(e):
            excluir_data(id)
            page.dialog.open = False
            atualizar_tabela_datas(); atualizar_metricas()
    ...
    def testar_email(e):
        ...
        if not email_teste:
            ...
            if dados:
                _, _, nome_teste, email_teste, *_ = dados[0]
            else:
    ...
    def calcular_metricas():
        return contar_prestadores(), contar_datas_ativas(), contar_falhas_7dias()
    ...
    def atualizar_tabela_datas():
        ...
        hoje = date.today()
        for row in listar_datas_envio(filtro_datas):
            id_, tipo, ref, fat_ini, fat_fim, rec_ini, rec_fim, status, *_ = row
            fat_fim_str = fmt_data(fat_fim); alerta = False
            if fat_fim:
                try:
                    dias = (datetime.strptime(str(fat_fim),"%Y-%m-%d").date() - hoje).days
                    if 0 < dias <= 5: fat_fim_str = f"⚠️ {fat_fim_str}"; alerta = True
                except: pass
            tabela_datas.rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(tipo)),
                ft.DataCell(ft.Text(ref, weight="bold")),
                ft.DataCell(ft.Text(fmt_data(fat_ini))),
                ft.DataCell(ft.Text(fat_fim_str, color=ft.Colors.RED_400 if alerta else None)),
                ft.DataCell(ft.Text(fmt_data(rec_ini))),
                ft.DataCell(ft.Text(fmt_data(rec_fim))),
                ft.DataCell(criar_badge_status(status)),
                ft.DataCell(ft.Row([
                    ft.IconButton(icon=ft.Icons.EDIT, icon_color=ft.Colors.BLUE_300,
                                  tooltip="Editar", on_click=lambda e, rid=id_: editar_data(rid)),
                    ft.IconButton(icon=ft.Icons.DELETE, icon_color=ft.Colors.RED_300,
                                  tooltip="Excluir", on_click=lambda e, rid=id_: excluir_data_dlg(rid)),
                ], spacing=5))
            ]))
        page.update()
    ...
    def atualizar_tabela_log():
        ...
        for data, prestador, ref, tipo_conta, notif, sucesso in listar_log_envios(100):
            tabela_log.rows.append(ft.DataRow(cells=[
                ...
                ft.DataCell(ft.Icon(
                    ...
                    color=ft.Colors.GREEN_400 if sucesso else ft.Colors.RED_400, size=20)),
            ]))
        page.update()
    ...
    btn_salvar_prestador = ft.ElevatedButton(
        text="Salvar Prestador", icon=ft.Icons.SAVE,
        bgcolor=UNIMED_GREEN, color="white", on_click=salvar_prestador_com_feedback,
        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)))
    btn_salvar_data = ft.ElevatedButton(
        text="Salvar Datas", icon=ft.Icons.SAVE,
        bgcolor=UNIMED_GREEN, color="white", on_click=salvar_data_com_feedback,
        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)))
    btn_novo_prestador = ft.ElevatedButton(
        text="Novo Prestador", icon=ft.Icons.ADD,
        on_click=lambda e: limpar_campos_prestador())
    btn_nova_data = ft.ElevatedButton(
        text="Nova Data", icon=ft.Icons.ADD,
        on_click=lambda e: limpar_campos_data())
    btn_testar_email = ft.ElevatedButton(
        text="Testar E-mail", icon=ft.Icons.EMAIL,
        bgcolor=ft.Colors.ORANGE_400, color="white", on_click=testar_email,
        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)))
    ...
    aba_prestadores = ft.Column([
        ...
        ft.ResponsiveRow([ft.Column([txt_filtro_prestadores], col={"sm": 12, "md": 12})]),
    ...
    aba_datas = ft.Column([
        ...
        ft.Container(
            ...
            padding=20, bgcolor=UNIMED_CARD_BG, border_radius=12),
        ft.Divider(color=UNIMED_GREEN, height=20),
        ft.ResponsiveRow([ft.Column([txt_filtro_datas], col={"sm": 12, "md": 12})]),
    ...
    aba_log = ft.Column([
        ft.Row([
            ...
            ft.Row([
                ft.IconButton(icon=ft.Icons.REFRESH, icon_color=UNIMED_GREEN,
                              tooltip="Atualizar", on_click=lambda e: atualizar_tabela_log()),
                btn_testar_email,
            ]),
    ...
    aba_dashboard = ft.Column([
        ...
        ft.Container(
            content=ft.ListView(controls=[tabela_log], expand=True),
            border=ft.border.all(1, UNIMED_GREEN), border_radius=12, padding=10, expand=True),
    ], expand=True, spacing=20)
    ...
    tabs = ft.Tabs(
        ...
        divider_color=UNIMED_GREEN, indicator_color=UNIMED_LIGHT)
    ...
    page.appbar = ft.AppBar(
        ...
        bgcolor=UNIMED_DARK, color="white", center_title=False,
        actions=[ft.IconButton(
            icon=ft.Icons.REFRESH, icon_color=UNIMED_LIGHT, tooltip="Atualizar tudo",
            on_click=lambda e: [
                atualizar_metricas(), atualizar_tabela_prestadores(),
                atualizar_tabela_datas(), atualizar_tabela_log()])])
